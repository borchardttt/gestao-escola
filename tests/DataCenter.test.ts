...
describe('DataCenter', () => {
  ...
  afterEach(() => {
    (dataCenter as any).students = [];
    (dataCenter as any).teachers = [];
    (dataCenter as any).classrooms = [];
  });
  ...
  test('should add a teacher', () => {
    const status: StatusTeacher = StatusTeacher.ativo;
  ...
  test('should throw an error if student or classroom is not found when adding a student to a classroom', () => {
    expect(() => dataCenter.addStudentToClassroom(999, 999)).toThrow(
      'Estudante ou Sala não encontrada'
    );
  });
});
